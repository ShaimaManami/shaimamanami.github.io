...
function meshPositionForWidth(width) {
  if (width <= 696) return { x: 14, y: 10 };
  if (width <= 1040) return { x: 18, y: 14 };
  return { x: 22, y: 16 };
}
...
function init() {
  ...
  var camera = new PerspectiveCamera(54, window.innerWidth / window.innerHeight, 0.1, 100);
  ...
  material.onBeforeCompile = function (shader) {
    uniforms = UniformsUtils.merge([shader.uniforms, { time: { value: 0 } }]);
    shader.uniforms = uniforms;
    shader.vertexShader = VERTEX_SHADER;
    shader.fragmentShader = FRAGMENT_SHADER;
  };

  var geometry = new SphereGeometry(32, 128, 128);
  ...
  function applyLights() {
    ...
    var directional = new DirectionalLight(0xffffff, theme === "light" ? 1.8 : 2.0);
    directional.position.set(100, 100, 200);
    var ambient = new AmbientLight(0xffffff, theme === "light" ? 2.7 : 0.4);
    lights = [directional, ambient];
    scene.add(directional, ambient);
  }
  applyLights();
  document.addEventListener("themechange", applyLights);

  var mountTime = Date.now();

  var ySpring = createSpring(0, { stiffness: 30, damping: 20, mass: 2 });
  var xSpring = createSpring(0, { stiffness: 30, damping: 20, mass: 2 });
  ...
  function handleResize() {
    ...
    var pos = meshPositionForWidth(width);
  ...
  function renderFrame() {
    if (uniforms) uniforms.time.value = 5e-5 * (Date.now() - mountTime);
    mesh.rotation.z += 0.001;
    mesh.rotation.x = ySpring.get();
    mesh.rotation.y = xSpring.get();
  ...
  function onMouseMove(e) {
    ...
    ySpring.set(normalized.y / 2);
    xSpring.set(normalized.x / 2);
  }
